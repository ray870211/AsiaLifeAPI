...
function editButtonClick(selectedTable, id) {
  function getImage(img_url) {
    ...
    fetch("../php/get_image.php", {
      ...
      .then((jsonData) => {
        ...
        $("#" + table_title[selectedTable] + "_image").attr(
          "src",
          "data:image/png;base64," + jsonData.image
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  getImage(
    $("#" + table_title[selectedTable] + "_tbody").children()[id % 10].children[2].textContent
  );
